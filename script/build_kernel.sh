...
if [ -e $LIB_DIR/bitcode/opencl.amdgcn.bc ]; then
  ...
  BC_DIR=$LIB_DIR
elif [ -e $ROCM_DIR/share/amdgcn/bitcode/opencl.bc ]; then
  BC_DIR=$ROCM_DIR/share/amdgcn/bitcode
else
...
for GFXIP in $TGT_LIST ; do
  ...
  $BIN_DIR/llvm/clang -cl-std=CL$OCL_VER -include $INC_DIR/opencl-c.h $BITCODE_OPTS -target amdgcn-amd-amdhsa -mcpu=$GFXIP $TEST_NAME.cl -o $DST_DIR/$OBJ_FILE
